                  ...
                  .export = exports) %doVersion% {
                    ...
                    for (i in 1:nOuter) {
                          ...
                          if (method == 'PLS') {
                           ...
                           # cat(mtryIn)
                            if((length(keep_variables)+mtryIn)<=ncol(xTrain)&
                               (all(keep_variables%in%colnames(xTrain)))&
                               (all(keep_variables%in%colnames(xVal)))){
                      ...
                      if (method == 'PLS') {
                        ...
                        incVarMin<-unique(c(keep_variables[keep_variables%in%colnames(xTrain)],incVarMin))
                        # min model
                        ...
                        # mid model
                        incVarMid<-unique(c(keep_variables[keep_variables%in%colnames(xTrain)],incVarMid))
                        ...
                        # max model
                        incVarMax<-unique(c(keep_variables[keep_variables%in%colnames(xTrain)],incVarMax))
