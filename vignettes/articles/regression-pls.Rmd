...
regrModel <- MUVR2(X = XRVIP2, Y = YR2, nRep = 35, nOuter = 8,
                   varRatio = 0.85, method = "PLS", modReturn = TRUE)
stopCluster(cl)
```
...
`plotVAL()` shows the validation metric (here RMSEP) against the number of
...
Every plot in MUVR2 also has a `ggplot2` version, which returns an object you can
theme, save or make interactive:
...
For regression, `plotMV()` puts the actual target on the x-axis and the
...
```{r mv, fig.show = "hold", out.width = "49%", fig.width = 5.5}
...
`biplotPLS()` overlays observation scores and variable loadings from the
...
```{r biplot, fig.show = "hold", out.width = "49%", fig.width = 5.5}
...
`plotStability()` is the check that tells you whether the model has converged. For
regression it draws three panels — number of selected variables, the proportion of
them that match the final selection, and Q2 — each as both a per-repetition series
and a cumulative average.
...
article's model uses `nRep = `r nRep_article`, deliberately far short of that, so
expect the curves to still be moving.

```{r stability, fig.height = 7}
plotStability(regrModel, model = "min")
```

The `ggplot2` version puts the same panels in facets:

```{r stability-gg, fig.height = 7}
ggplotStability(regrModel, model = "min")
```

## Which variables?

`plotVIRank()` shows the rank of each variable across repetitions, as a boxplot;
...
```{r virank, fig.show = "hold", out.width = "49%", fig.width = 5.5, fig.height = 6}
